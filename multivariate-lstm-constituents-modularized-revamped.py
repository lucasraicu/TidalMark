...
COMPUTE_CONSTITUENTS_SCRIPT = os.path.join(SCRIPT_DIR, "compute_constituents.py")
STEP_MINUTES = 6
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--disable-lr-adapt", action="store_true")
    parser.add_argument("--sweep", action="store_true")
    parser.add_argument("--data", type=str, default=None)
    parser.add_argument(
        "--model", choices=["lstm", "bilstm", "conv-lstm", "gru", "attn-lstm"]
    )
    ...
    parser.add_argument(
        "--horizons", nargs="+", type=int, default=DEFAULT_HORIZONS_DAYS
    )
    parser.add_argument("--num-workers", type=int, default=4)
    parser.add_argument("--base-output", type=str, default=None)
    ...
    if args.base_output is None:
        args.base_output = DEFAULT_BASE_OUTPUT
...
def load_and_preprocess_data(path: str, station_id: str) -> str:
    print(f"Loading harmonics data for {station_id}...")
    station_file = os.path.join(DEFAULT_BASE_OUTPUT, f"{station_id}.tsv")
    harmonic_file = os.path.join(DEFAULT_BASE_OUTPUT, f"{station_id}_harmonics.tsv")
    out_path = os.path.join(DEFAULT_BASE_OUTPUT, f"{station_id}_with_harmonics.tsv")
    ...
    if not os.path.exists(harmonic_file):
        cmd = [
            "python3",
            COMPUTE_CONSTITUENTS_SCRIPT,
            "--station-id",
            station_id,
            "--out-dir",
            DEFAULT_BASE_OUTPUT,
    ...
    with open(station_file, "r") as f:
        df_main = pd.read_csv(station_file, sep="\t", parse_dates=["time"])

    with open(harmonic_file, "r") as f:
        df_harm = pd.read_csv(harmonic_file, sep="\t", parse_dates=["time"])

    df_main.rename(columns={"value": "main"}, inplace=True)

    df_main["time"] = (
        pd.to_datetime(df_main["time"]).dt.round("min").dt.tz_localize("UTC")
    )
    df_harm["time"] = pd.to_datetime(df_harm["time"]).dt.round("min")

    # If harmonics already has timezone info, convert it
    if df_harm["time"].dt.tz is not None:
        df_harm["time"] = df_harm["time"].dt.tz_convert("UTC")
    else:
        df_harm["time"] = df_harm["time"].dt.tz_localize("UTC")

    merged = pd.merge(df_main, df_harm, on="time", how="inner").dropna()
...
def train_and_evaluate(args):
    ...
    harmonic_cols = [c for c in merged_df.columns if c not in ["time", "main"]]
    features_array = merged_df[["main"] + harmonic_cols].values.astype(np.float32)
